...
from autotrader.datasource.database.stock_schema import Filter
...
class RecreateFilters:
    ...
    def delete_old_filter(self):
        ...
        for my_filter in self.arguments["filters"]:
            ...
            self.db_tool.session.query(Filter).\
                filter(Filter.name == my_filter).delete()
        self.db_tool.commit()
    ...
    def build(self):
        ...
        if from_date is None or to_date is None:
            to_date = self.db_tool.session.query(Filter.date).order_by(Filter.date.desc()).first()
            from_date = self.db_tool.session.query(Filter.date).order_by(Filter.date.asc()).first()
